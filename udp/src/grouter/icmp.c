...
//A global value to keep track of the TTL
int TTLtrack=1;
...
case ICMP_TTL_EXPIRED:
		verbose(2, "[ICMPProcessPacket]:: ICMP processing for TTL Expired");
		ICMPProcessingTTLExpired(in_pkt);
		break;

case ICMP_PORT_UNREACH:
		verbose(2, "[ICMPProcessPacket]:: ICMP processing for Port Unreachability");
		UDPtraceroutereply(in_pkt);
		break;
...
/*Process a TTL expired Packet*/

void ICMPProcessingTTLExpired(gpacket_t *in_pkt)
{
int destination;
...
//printf ("\n Source is %d",ntohs(udp_packet->udp_src));
//printf("\n Length is %d",ntohs(udp_packet->udp_len));
//printf("\n Destination is  %d ",ntohs(udp_packet->udp_dst));
//printf("\n Checksum is %d %x",ntohs(udp_packet->udp_chk),ntohs(udp_packet->udp_chk));
//destination = ntohs(udp_packet->udp_dst);
//printf("Destination port is  %d",destination);
//printf("\nReceived expired TTL packet");
///udptraceroute(ipsecondpacket->ip_dst, 20, 1,destination+3);
//printf("%s",IP2Dot(tmpbuf,ipsecondpacket->ip_dst));
udpTTLexpired(gNtohl(tmpbuf,ipsecondpacket->ip_dst),gNtohl(tmpbuf+20,ipkt->ip_src), ntohs(udp_packet->udp_dst));
}
...
void ICMPDoPing(uchar *ipaddr, int pkt_size, int retries)
{
	...
	if (ping_active == 0)
	{
		printf("Pinging nishanth IP Address [%s]\n", IP2Dot(tmpbuf, ipaddr));
		ping_active = 1;
...
void DestinationUnreachableICMP(gpacket_t *in_pkt)
{
	ip_packet_t *ipkt = (ip_packet_t *)in_pkt->data.data;
	int iphdrlen = ipkt->ip_hdr_len *4;
	icmphdr_t *icmphdr = (icmphdr_t *)((uchar *)ipkt + iphdrlen);
	...
	verbose(2, "[ICMPProcessDestinationunreachable]:: Sending... ICMP Destination port unreacable message ");
...
void ICMPProcessEchoReply(gpacket_t *in_pkt)
{
	...
	if (icmphdr->type == ICMP_ECHO_REPLY)
	{
		...
		printf("%d recieved nsihanth bytes from %s: icmp_seq=%d ttl=%d time=%6.3f ms\n", 
