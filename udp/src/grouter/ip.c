...
//printf ("%d\n",in_pkt->frame.src_interface);
//printf ("%s\n",in_pkt->data.data);
//printf ("%s\n",in_pkt->frame.src_ip_addr);
ip_packet_t *ip_packet=(ip_packet_t *)in_pkt->data.data;
printf("\n%d  %d",ip_packet->ip_ttl,ip_packet->ip_prot);
printf("\n %d ",ip_packet->ip_src);
printf("\n %s ",IP2Dot(tmpbuf, gNtohl((tmpbuf+20), ip_packet->ip_src)));
udp_packet_t *udp_packet=(udp_packet_t *)ip_packet->data;
printf("\n Destination is  %d ",ntohs(udp_packet->udp_dst));
//Check whether the destination port is meant for traceroute
...
int IPOutgoingUDPPacket(gpacket_t *pkt, uchar *dst_ip, int size, int newflag, int src_prot,int TTL)
{
        ip_packet_t *ip_pkt = (ip_packet_t *)pkt->data.data;
	ushort cksum;
	char tmpbuf[MAX_TMPBUF_LEN];
	...
	ip_pkt->ip_ttl = TTL;                        // set TTL to default value
	ip_pkt->ip_cksum = 0;                       // reset the checksum field
	ip_pkt->ip_prot = src_prot;  // set the protocol field
...
void get_the_IP_address(gpacket_t *pkt,uchar *ip_addr)
{
	char tmpbuf[MAX_TMPBUF_LEN];
	uchar iface_ip_addr[4];
	int status;
 	...
 	ip_packet_t *ip_pkt = (ip_packet_t *)pkt->data.data;
	COPY_IP(ip_pkt->ip_dst, gHtonl(tmpbuf, ip_addr));
	if (findRouteEntry(route_tbl, gNtohl(tmpbuf, ip_pkt->ip_dst),
				   pkt->frame.nxth_ip_addr, &(pkt->frame.dst_interface)) == EXIT_FAILURE)
				   return EXIT_FAILURE;
