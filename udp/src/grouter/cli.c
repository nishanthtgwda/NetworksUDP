...
int CLIInit(router_config *rarg)
{
	...
	registerCLI("traceroute", tracerouteCmd, SHELP_TRACEROUTE, USAGE_TRACEROUTE, LHELP_TRACEROUTE);  // Check
	registerCLI("nc",sendCmd,SHELP_TRACEROUTE, USAGE_TRACEROUTE, LHELP_TRACEROUTE);  // Check
...
//Called when the main nc is invoked
void sendCmd()
{
char *next_tok = strtok(NULL, " \n");
int tries,i,dport,sport=0,bytes_sent,lport,cport;
char buffer[50], tmpbuf[60],close[]="close";
uchar ip_addr[4];

//If a user wants to close a particular port which has been opened for listening
if(strcmp(next_tok,close)==0)
{
	next_tok = strtok(NULL, "\n");
	cport= gAtoi(next_tok);
	closeport(cport);
	printf("Closed port %d\n",cport);
}
//Checks for a condition is the port is opened for listening
else if(next_tok[0]=='-'&&next_tok[1]=='l')
{
	next_tok = strtok(NULL, "\n");
	lport= gAtoi(next_tok);
	receiveCmd(lport);
	printf("Opened port number %d\n",lport);
}
//When a IP address follows a nc command(intended for sending a UDP packet)
else
{
	int size,socketid;
	Dot2IP(next_tok,ip_addr);
	next_tok = strtok(NULL, " \n");
	if(next_tok[0]=='-' && next_tok[1]=='p')
	{
		next_tok = strtok(NULL, " \n");
		dport= gAtoi(next_tok);
	}
	next_tok = strtok(NULL, " \n");
	if(next_tok!=NULL)
		if(next_tok[0]=='-' && next_tok[1]=='s')
		{
			next_tok = strtok(NULL, " \n");
			sport=gAtoi(next_tok);
		}
		else 
		{
		}
	...
	size=i;
	//If no source port is specified, use a random port
	if(sport==0)
		socketid=assign_socket(2);
	else
	{
		int result=checkforopenport(sport);
		if(result==1) //Checking whether a port is currently in use
		{
			printf("\nCurrently the port is in use\n");
			return;
		}
		socketid=assign_socket(2);
		bindport(socketid,sport);
	}
	//If no sockets are free to be allocated
	if(socketid==0)
	{
	...
	{
		bytes_sent=outputprocessingudp(socketid,buffer,size,ip_addr,dport,0);
		freesocket(socketid); // Free the socket after the operation
...
void receiveCmd(int port)
{
	int status,socketid;
	int result=checkforopenport(port);
	if(result==1) //Checking whether a port is currently in use
	{
		printf("Currently the port is in use\n");
		return;
	}
	int queue=createqueue(port); //Create a queue for the port
	pthread_t threadid;
	socketid=assign_socket(2); //Assign a socket for the port
	bindport(socketid,port);   		//Bind the socket to the port
	status=pthread_create(&(threadid), NULL, (void *)scanningfunction, (void *)queue); //Create a thread which continously scans the queue for any packet received
	return;
}


void *scanningfunction(int queue)
{
	scanningfunctioninudp(queue);
}
...
void tracerouteCmd()
{
	...
	Dot2IP(next_tok, ip_addr);
	verbose(2, "[tracerouteCmd]:: traceroute command sent, tries = %d, IP = %s",
		tries, IP2Dot(tmpbuf, ip_addr));


	udptraceroute(ip_addr);
...
void parseACLICmd(char *str)
{
	...
	{
		printf("WARNING: %s nishanth not a gRouter command (deferring to Linux)\n", token);
		system(orig_str);
	}
}
